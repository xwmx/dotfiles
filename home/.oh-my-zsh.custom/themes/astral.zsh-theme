...
# _astral_git_prompt()
#
# Display either the default git prompt or one generated by `git-radar`
#
# git-radar
#
...
_astral_git_prompt() {
  local _prompt_section
  if hash "git-radar" &> /dev/null
  then
    _prompt_section="$(git-radar --zsh --fetch)"
  else
    _prompt_section="$(git_prompt_info)"
  fi
  printf "%s\n" "$_prompt_section"
}
